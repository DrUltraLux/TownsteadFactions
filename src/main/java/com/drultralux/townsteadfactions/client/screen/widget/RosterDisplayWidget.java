...
/**
 * A draggable widget that lists each member of the player's currently
 * assigned faction, along with their origin and resolved title.
 */
public class RosterDisplayWidget extends DraggableWidget {
     ...
     * roster entries (name, origin, and title) for the player's current
     * faction, unless the widget is minimized.
     *
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        boolean hasAnyEntries = factionData != null &&
                ((factionData.roster != null && !factionData.roster.isEmpty()) ||
                        (factionData.villagerRoster != null && !factionData.villagerRoster.isEmpty()));

        if (hasAnyEntries) {
            for (RosterEntry entry : factionData.roster.values()) {
                if (entry == null || entry.name() == null || entry.name().trim().isEmpty()) continue;
                String line = "§7• §f" + entry.name() + " §7(" + entry.root() + ") §e" + entry.title();
            ...
            for (RosterEntry entry : factionData.villagerRoster.values()) {
                if (entry == null || entry.name() == null || entry.name().trim().isEmpty()) continue;
                if (currentYOffset >= this.y + this.height - 8) break;
                String line = "§7• §7" + entry.name() + " §7(" + entry.root() + ") §e" + entry.title();
                graphics.drawString(this.font, Component.literal(line), this.x + 10, currentYOffset, FactionPalette.getBarColor("text_muted"), false);
                currentYOffset += 10;
            }
        } else {
